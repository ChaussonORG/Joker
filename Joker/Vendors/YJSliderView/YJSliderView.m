...
- (void)bindStyleButton:(UIButton *)btn redNum:(NSInteger)redNum status:(BOOL)isSelected {
    ...
    [self.titleButton setTitleColor:isSelected ?  [JKStyleConfiguration blackColor] ? [JKStyleConfiguration blackColor] : [JKStyleConfiguration bbbbbbColor] : [JKStyleConfiguration bbbbbbColor] forState:UIControlStateNormal];
...
- (void)setUpButtons {
    ...
    for (NSInteger index = 0; index < [self.delegate numberOfItemsInYJSliderView:self]; index ++) {
        ...
        [btn setTitleColor: [JKStyleConfiguration blackColor] ? [JKStyleConfiguration blackColor] : [JKStyleConfiguration bbbbbbColor] forState:UIControlStateNormal];
...
//        make.left.mas_equalTo(preStartPointX + (startPointX - preStartPointX) * rate);
        make.left.mas_equalTo(cellFrame.origin.x + (cellFrame.size.width - 20) / 2);
    }];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (type) {
        case TITLE: {
            ...
            cell.themeColor = [JKStyleConfiguration blackColor];
...
- (void)updateLabelInCellAtIndex:(CGFloat)index nextIndex:(CGFloat)nextIndex {
    ...
    [self changeButtonColor:nextBtn To:[JKStyleConfiguration blackColor] ? [JKStyleConfiguration blackColor] : [JKStyleConfiguration bbbbbbColor] WithScale:rate];
    [self changeButtonColor:currentBtn To:[JKStyleConfiguration bbbbbbColor] WithScale:rate];
...
- (UICollectionView *)titleCollectionView {
    if (_titleCollectionView == nil) {
        ...
        _titleCollectionView.backgroundColor = [JKStyleConfiguration whiteColor];
    ...
    if ( totalNum <= 5) {
        ...
        topLayout.itemSize = CGSizeMake(self.frame.size.width / 5, topViewHeight);
    }
...
- (UIView *)sliderLine {
    if (_sliderLine == nil) {
        ...
        _sliderLine.backgroundColor = [JKStyleConfiguration blackColor] ? [JKStyleConfiguration blackColor] : [JKStyleConfiguration bbbbbbColor];

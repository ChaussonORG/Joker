//
//  JKAnimationTimeLineController.m
//  Joker
//
...
#import "JKAnimationTimelineCell.h"
@interface JKAnimationTimeLineController ()<UITableViewDelegate,UITableViewDataSource,ChooseTopicDelegate>
...
@implementation JKAnimationTimeLineController
- (instancetype)init
{
    self = [super init];
    if (self) {
        
        self.viewModel = [[JKAnimationTimeLineVM alloc]init];
    }
    return self;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (self.viewModel.type == JKAnimationCurrent) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    JKAnimationTimelineCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (cell == nil) {
        cell = [[JKAnimationTimelineCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
...
- (void)chooseTopicWithIndex:(NSInteger)index{
    ...
    if (index == 0) {
        if (self.viewModel.type ==JKAnimationFuture) {
            self.viewModel.type = JKAnimationCurrent;
        }
        else{
        ...
        if (self.viewModel.type ==JKAnimationCurrent) {
            self.viewModel.type = JKAnimationFuture;
        }
        else{

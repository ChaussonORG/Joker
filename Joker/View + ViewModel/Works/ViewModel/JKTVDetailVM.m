...
@interface JKTVDetailVM()<WorkrefreshSuperTableViewDelegate,CHLoginModalControllerDelegate,WorkCommentedDelegate>
...
- (void)requestData{
    ...
    JKTVDetailApi *api = [[JKTVDetailApi alloc]initWithWorkId:self.workId];
    
    [api startWithSuccessBlock:^(__kindof JKTVDetailApi *request) {
        ...
        if (self.imageArrs.count%4 == 0) {
            self.imagesCellHeight =(self.imageArrs.count/4 )*((ScreenWidth - 35)/4 + 10) +50;
        }
        else{
            
            if (self.imageArrs.count%4 < 4) {
                self.imagesCellHeight =(self.imageArrs.count/4 + 1)*((ScreenWidth - 35)/4 + 10)+50;
            }
            else{
    ...
    } failureBlock:^(__kindof JKTVDetailApi *request) {
...
- (void)requestMoreData{
    ...
    if (self.filterType == JKTVDataComment) {
        
        [self  requestMoreCommentData];
    }
    else if(self.filterType == JKTVDataTopic){
...
- (void)requestMoreTopicData{
    ...
    JKTopicsListApi *api = [[JKTopicsListApi alloc]initTopicTV];
    api.projectId = self.workId;
